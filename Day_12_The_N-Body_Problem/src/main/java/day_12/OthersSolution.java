...
/**
 * Source: https://github.com/WillemDeRoover/advent-of-code/blob/master/src/day12/Puzzle12.java
 */
public class OthersSolution {

  public static void main(String[] args) {
    List<Coordinate> xAxis = createAxis(-8, -5, 11, 1);
    List<Coordinate> yAxis = createAxis(-9, 2, 8, -4);
    List<Coordinate> zAxis = createAxis(-7, -1, -14, -11);
  ...
  private static void calculateEnergyOfSystem(
      List<Coordinate> xAxis, List<Coordinate> yAxis, List<Coordinate> zAxis) {
    ...
    System.out.println(
        "The total energy of the system after 1000 cycles: "
            + calculateEnergy(xAxis, yAxis, zAxis));
  }
  ...
  private static void moveOneStep(List<Coordinate> axis) {
    axis.forEach(
        anAxis ->
            axis.stream()
                .filter(otherAxis -> !otherAxis.equals(anAxis))
                .forEach(anAxis::recalculateVelocity));
  ...
  private static int calculateEnergy(Coordinate x, Coordinate y, Coordinate z) {
    return (Math.abs(x.getPosition()) + Math.abs(y.getPosition()) + Math.abs(z.getPosition()))
        * (Math.abs(x.getVelocity()) + Math.abs(y.getVelocity()) + Math.abs(z.getVelocity()));
  }

  private static void calculateStepsForCycle(
      List<Coordinate> xAxis, List<Coordinate> yAxis, List<Coordinate> zAxis) {
    BigInteger stepsForCycle =
        lcd(
            lcd(calculateStepsForCycle(xAxis), calculateStepsForCycle(yAxis)),
            calculateStepsForCycle(zAxis));
